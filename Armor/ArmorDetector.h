...
*   结构体用来存放一些算法用到的常量并用构造函数初始化
*/
struct ArmorParam
{
    //Pre-treatment
    
    int brightness_threshold; //阈值
    int color_threshold;

    float light_color_detect_extend_ratio;//面积扩大倍数
    ...
    */
    ArmorParam()
    {
        ...
        light_max_height_diff_ratio_ = 0.2f; //0.5
...
*   LightDescriptor 类用来描述检测到的灯条
*   对RotatedRect类的封装,也就是把灯条看成它的外接矩形
*/
class LightDescriptor
{
public:
    LightDescriptor() {}
    LightDescriptor(const RotatedRect& light) //RotatedRect旋转矩形类
    {
...
public:
    ...
    *   暂时不用
    */
    void getFrontImg(const Mat& grayImg);
...
public:
    array<RotatedRect, 2> lightPairs; //0 left, 1 right  左灯和右灯
    float sizeScore;		//S1 = e^(size) 装甲板的大小
    float distScore;		//S2 = e^(-offset) 装甲板距离视野中心的距离
    float rotationScore;		//S3 = -(ratio^2 + yDiff^2) 装甲板的视野正对程度
    float finalScore; //综合得分

    vector<Point2f> vertex; //four vertex of armor area, lihgt bar area exclued!!四个顶角坐标
    Mat frontImg; //front img after prespective transformation from vertex,1 channel gray img
...
public:
    vector<Mat> _small_Armor_template;
    vector<Mat> _big_Armor_template;
    ArmorDetector();
    ...
    *	@Others: API for client
    */
    const vector<Point2f> getArmorVertex() const;
...
private:
    ...
    Rect _roi; //relative coordinates

    Mat _srcImg; //source img
    Mat _roiImg; //roi from the result of last frame 感兴趣的区域
    Mat _grayImg; //gray img of roi
